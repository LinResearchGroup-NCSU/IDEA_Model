...
def phi_pairwise_contact_well(res_list_tmonly, res_list_entire, neighbor_list, parameter_list, CPLEXmodeling=False, CPLEX_name='IDK', Decoy_flag=False):
    ...
    for res1globalindex, res1 in enumerate(res_list_entire):
        ...
        if CPLEXmodeling:
            if (res1 in res_list_tmonly):
                if ( Decoy_flag == False ): 
                    for res2 in get_neighbors_within_radius_native(neighbor_list, res1, r_max + 2.0):
                        ...
                        if (CPLEX_name == '1nkp'):                       
                            if (res2chain == 'A'):
                                ...
                                rij = get_interaction_distance_native(res1, res2)
                ...
                elif ( Decoy_flag == True ): 
                    for res2 in get_neighbors_within_radius_decoy(neighbor_list, res1, r_max + 2.0):
                        res2index = get_local_index(res2)
                        res2chain = get_chain(res2)
                        res2globalindex = get_global_index(res_list_entire, res2)
                        # Here, we strictly consider only between the DNA chain and the protein chain:
                        ...
                        if (CPLEX_name == '1nkp'):                       
                            if (res2chain == 'A'):
                                res1type = get_res_type(res_list_entire, res1)
                                res2type = get_res_type(res_list_entire, res2)
                                rij = get_interaction_distance_decoy(res1, res2)
                                phi_pairwise_contact_well[res1type][res2type] += interaction_well(rij, r_min, r_max, kappa)
                                if not res1type == res2type:
                                    phi_pairwise_contact_well[res2type][res1type] += interaction_well(rij, r_min, r_max, kappa)
...
#        if not number_of_lines_in_file >= 1:
        ...
        phis_to_write = phi(res_list_tmonly_native, res_list_entire_native,
                            neighbor_list, parameters, CPLEXmodeling=CPLEXmodeling, CPLEX_name=CPLEX_name, Decoy_flag=False)
...
#        if not number_of_lines_in_file >= max_decoys:
        ...
        for i_decoy, decoy_sequence in enumerate(decoy_sequences):
            ...
            phis_to_write = phi(res_list_tmonly, res_list_entire,
                                neighbor_list, parameters, CPLEXmodeling=CPLEXmodeling, CPLEX_name=CPLEX_name, Decoy_flag=True)
            output_file.write(str(phis_to_write).strip(
                '[]').replace(',', ' ') + '\n')
        output_file.close()
...
evaluate_phis_over_training_set("proteins_list.txt", "phi1_list.txt", decoy_method='CPLEX_randomization', 
                                max_decoys=100000, tm_only=False, num_processors=1, CPLEXmodeling=True, CPLEX_name='1nkp')
