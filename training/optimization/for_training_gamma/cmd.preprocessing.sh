...
cp ../../../../prepare/tcr3d/TCR_complexes/HLA0201/$PDBid.trunc.fitRenumberResidues.pdb native_structures_pdbs_with_virtual_cbs/native.pdb
...
rm randomize_position_peptide.txt
for ((i=$pResid_startID; i<$pResid_endID; i++))
do 
    printf "%d " $i >> randomize_position_peptide.txt
done
printf "%d" $pResid_endID >> randomize_position_peptide.txt


mkdir -p TCR_randomization 
mv randomize_position_peptide.txt native.seq gBinder_sequences.txt TCR_randomization/
